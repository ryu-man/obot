<script lang="ts">
	...
	let listUsageStatsResponse = $state<AuditLogUsageStats>();

	let listAuditLogsResponse = $state<PaginatedResponse<AuditLog>>();
	let isSpinnerVisible = $state(false);
	...
	async function fetchLogsAndUsers(filters?: AuditLogFilters, offset?: number, limit?: number) {
		...
		if (mcpId) {
			...
			listAuditLogsResponse = await AdminService.listServerOrInstanceAuditLogs(
				mcpId,
				sortAndFilters
			);

			listUsageStatsResponse = await AdminService.listServerOrInstanceAuditLogStats(mcpId, {
			...
			listAuditLogsResponse = await AdminService.listAuditLogs({
				...sortAndFilters,
				mcpServerCatalogEntryName: mcpCatalogEntryId,
				mcpServerDisplayName
			});

			listUsageStatsResponse = await AdminService.listAuditLogUsageStats({
				startTime: filters?.startTime ?? '',
				endTime: filters?.endTime ?? '',
				mcpServerCatalogEntryName: mcpCatalogEntryId,
				mcpServerDisplayName
...
<div class="relative flex flex-col gap-4">
	{#if !listUsageStatsResponse || !listAuditLogsResponse || isSpinnerVisible}
		<div
			class="pointer-events-none absolute inset-0 z-50 flex w-full justify-center bg-black/25 py-52"
			transition:fade={{ duration: 200 }}
		>
			<div class="sticky top-0">
				<LoaderCircle class="text-on-background size-6 animate-spin" />
			</div>
		</div>
	{/if}

	{#if listAuditLogsResponse}
		{#if listUsageStatsResponse}
			{@const totalCalls = listUsageStatsResponse?.totalCalls ?? 0}
			{@const uniqueUsers = listUsageStatsResponse?.uniqueUsers ?? 0}
			...
			<div class="flex flex-col gap-4">
				...
				>
					{#if graphView === 'calls'}
						{@const callsGraphData = compileCallsGraphData(listAuditLogsResponse?.items ?? [])}
						{#if callsGraphData.length > 0}
							{@const timeRange = (() => {
								const dates =
									listAuditLogsResponse?.items?.map((log) => new Date(log.createdAt)) ?? [];
						...
						{/if}
					{:else}
						{@const graphData = compileBarGraphData(listUsageStatsResponse, graphView)}
		...
		{@const auditLogs = (listAuditLogsResponse?.items ?? [])
			.map((auditLog) => {
		...
		{#if allowPagination}
			{@const total = listAuditLogsResponse?.total ?? 0}
			<div
