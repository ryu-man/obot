...
<DraggableItem
	as="li"
	class={twMerge('w-full gap-2', isTaskRunning && !isRunning && 'opacity-50')}
	{index}
	id={step.id}
	data={step}
>
	<div class="flex w-full items-start justify-between gap-6">
		<div class="flex flex-1 grow flex-col gap-0">
			...
			{#if shouldShowOutput && messages.length}
				<div
					class="transition-height relative my-3 -ml-4 box-content flex min-h-6 flex-col gap-4 overflow-hidden rounded-lg bg-transparent p-5 dark:bg-black"
